...
describe("layer", () => {
  ...
  describe("collectLayerPackages", () => {
    ...
    it("should not include packages without resolved path", () => {
      const result = collectLayerPackages(tempDir, ["pkg-a", "pkg-b"]);

      // Every package in the list should have a resolved path
      for (const pkg of result.packages) {
        const pkgPath = result.resolvedPaths.get(pkg);
        expect(pkgPath).toBeDefined();
        expect(typeof pkgPath).toBe("string");
      }
    });
