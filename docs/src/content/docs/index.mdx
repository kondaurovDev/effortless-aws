...
<Tabs>
  ...
  <TabItem label="HTTP API">
    One export — one API. `get` routes handle queries, `schema` + `post` handle commands.
    ...
    export const todoApi = defineApi({
      ...
      get: {
        "/": async ({ deps }) => ({
          status: 200,
          body: await deps.todos.query({ pk: "TODO" }),
        }),
        "/{id}": async ({ req, deps }) => {
          const todo = await deps.todos.get({ pk: "TODO", sk: req.params.id });
